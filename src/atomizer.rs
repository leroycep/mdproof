...
use cmark::{CowStr, Event as ParseEvent, Parser, Tag};
...
#[derive(Debug)]
pub enum Atom<'src> {
    Text { text: CowStr<'src>, style: Style },
    Image { uri: CowStr<'src> },
}
...
impl<'src> Atomizer<'src> {
    ...
    fn split_text(&mut self, text: CowStr<'src>) -> (Option<Event<'src>>, AtomizerState<'src>) {
    ...
    fn parse_event(
        ...
        match event {
            ...
            ParseEvent::Code(text) => {
                let mut style = self.current_style.clone();
                style.insert(Class::Code);
                return (
                    Some(Event::Atom(Atom::Text { text, style })),
                    AtomizerState::Parsing,
                );
            ...
            ParseEvent::Start(Tag::Image(_link_type, uri, _title)) => {
            ...
            ParseEvent::End(Tag::Image(_link_type, _url, _title)) => {
